...
def load_b3():
    ...
    model = build_model(cnn_net)
    model.load_weights('D:\DR\Diabetic-Retinopathy - Copy\src\webapp\models\effnet_b3.h5')
    return model 

def load_b5():
    cnn_net = efn.EfficientNetB5(weights='imagenet',include_top=False,input_shape=(456, 456, 3))
    model = build_model(cnn_net)
    model.load_weights('D:\DR\Diabetic-Retinopathy - Copy\src\webapp\models\effnet_b5.h5')
    return model

def load_b5_old():
    cnn_net = efn.EfficientNetB5(weights='imagenet',include_top=False,input_shape=(456, 456, 3))
    model = build_model(cnn_net)
    model.load_weights('D:\DR\Diabetic-Retinopathy - Copy\src\webapp\models\effb5_old_new.h5')
    return model

def load_b5_prc():
    cnn_net = efn.EfficientNetB5(weights='imagenet',include_top=False,input_shape=(380, 380, 3))
    model = build_model(cnn_net)
    model.load_weights('D:\DR\Diabetic-Retinopathy - Copy\src\webapp\models\effb5_old_new_preprocess.h5')
    return model
